...
namespace TestEstante
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Estante.SerializarEstante(est1, "INFOESTANTE1");
            Estante.SerializarEstante(est2, "INFOESTANTE2");

            Console.ReadLine();
            
        }

        private static int OrdenarProductos(Producto proUno, Producto proDos)//compara entre 2 string para ordenar en forma decreciente
        {
            int retorno=0;


             retorno= String.Compare(((int)proUno).ToString(), ((int)proDos).ToString());
